...
const fetchPaymentDetailsByInvoiceId = async (invoiceId: string) => {
if (!authToken) return;
setProcessingPaymentId(0); // Using 0 as a general loading indicator
try {
const res = await fetch(`http://51.75.119.133:8080/api/Payment/sales-txn/${invoiceId}`, {
headers: {
'Authorization': `Bearer ${authToken}`,
'Accept': 'application/json'
},
...
showToast('No payment details found for this invoice', 'info');
}
} catch (e: any) {
...
result = { success: true, message: 'Payment approved successfully' };
}
...
console.error('Failed to parse JSON response:', parseError);
result = { success: true, message: isApproved ? 'Sale approved successfully' : 'Sale rejected successfully' };
}
...
<div className="flex items-center justify-center gap-2">
<RippleButton
onClick={() => fetchPaymentDetailsByInvoiceId(payment.salesRequestId.toString())}
